...
"""
DEFINE_string(flag_name, default_value, docstring)(DEFINE_type)
...
tf.app.flags.DEFINE_integer('epoch', 4, 'Number of epoches')
...
def inference(image):
    ...
    with tf.Session() as sess:
        ...
        predict = np.ndarray.flatten(predict_index)
    return predict_val, predict_index ,predict[0]
...
def find_char(word):
    ...
    cur = conn.cursor()
    sql = "SELECT words_explain FROM cnword where words = '"+ word +"'"
    cur.execute(sql)
...
def image_processing(path):
    ...
    with tf.Session() as sess:
        # 输出解码之后的三维矩阵。
        for i in img_data.eval():
            if i.sum() != width * 3 * 255:
                xValuemin = countX
                # print("xValuemin:", xValuemin)
                break
            countX = countX + 1

        countX = height
        for i in img_data.eval()[::-1]:
            if i.sum() != width * 3 * 255:
                xValuemax = countX
                # print("xValuemax:", xValuemax)
                break
            countX = countX - 1

        img_data_transposed = tf.image.transpose_image(img_data)

        for i in img_data_transposed.eval():
            if i.sum() != height * 3 * 255:
                yValuemin = countY
                # print("yValuemin:", yValuemin)
                break
            countY = countY + 1

        countY = width
        for i in img_data_transposed.eval()[::-1]:
            if i.sum() != height * 3 * 255:
                yValuemax = countY
                # print("yValuemax:", yValuemax)
                break
            countY = countY - 1

        box = (yValuemin, xValuemin, yValuemax, xValuemax)
        path = filepath + '/' + shotname + '(Treated)' + extension
        roi = img.crop(box)
        roi.convert('RGB').save(path)
        return path
        # plt.imshow(roi)
        # plt.show()
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        print("Total Time of train is:",end-start)
        ...
        start = time.time()
        for line in open("./FilePath.txt"):
            image_path = line
        (filepath, tempfilename) = os.path.split(image_path);
        (shotname, extension) = os.path.splitext(tempfilename);
        if "(Treated)" not in shotname:
            image_path = image_processing(image_path)
            print(image_path)
        final_predict_val, final_predict_index,final_predict = inference(image_path)
        str = information(final_predict)
        print("The word in chiness is:",str)
        find_char(str)
        end = time.time()
        print("Total Time of train is:",end-start)
